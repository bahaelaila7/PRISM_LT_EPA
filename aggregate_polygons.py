...
def aggregate_sql(from_table, epa_level= 4):
    if epa_level not in [3,4]:
        raise Exception("Only epa levels 3 or 4 are allowed")
    is_monthly = lambda s: s if from_table == 'monthly' else ''
    output_table_name = f'EPA_L{epa_level}_{from_table}'
    ...
    SELECT m.{epa_attr}, m.YEAR{is_monthly(', m.MONTH')}, u.PXL_COUNT, 
...
if __name__ == '__main__':
    with sqlite3.connect('prism_data_coalesced.db') as conn:
        ...
        for epa_level in [3,4]:
            ...
            print('\tMONTHLY')
            table, sql= aggregate_sql(from_table = 'monthly', epa_level = epa_level)
            print('\t',table)
            cur.executescript(sql)
            print('\tYEARLY')
            table, sql= aggregate_sql(from_table = 'yearly', epa_level = epa_level)
            print('\t',table)
            cur.executescript(sql)
